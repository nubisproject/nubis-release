...
generate_changelog () {
    ...
    # Due to GitHub API Abuse triggers we will pull just the last 100 issues
    #+ so we no longer need to calculate this number
    # I (jd) am leaving this code commented out for now as this may change
    #+ in the future
    _ISSUE_COUNT='100'
...
#     log_term 2 "Setting _GITHUB_URL to: \"${_GITHUB_URL}\""
#     log_term 3 "File: '${BASH_SOURCE[0]}' Line: '${LINENO}'"
#
#     # Collect the JSON list of issues
#     local _ISSUE_LIST; _ISSUE_LIST=$(collect_data "${_GITHUB_URL}")
...
#     log_term 3 "File: '${BASH_SOURCE[0]}' Line: '${LINENO}'"
    ...
    # Set up github_changelog_generator command
    CHANGELOG_FILE='CHANGELOG.md'
    declare -a CHANGELOG_GENERATOR_COMMAND
    CHANGELOG_GENERATOR_COMMAND+=( 'github_changelog_generator' )
    # Pass the oath token
    CHANGELOG_GENERATOR_COMMAND+=(  '--token' "${GITHUB_OATH_TOKEN}" )
    # Set the future-release to capture current (unreleased) issues
    CHANGELOG_GENERATOR_COMMAND+=( '--future-release' "${_RELEASE}" )

    # Set up the GtHub API URL for the latest release
    local _GITHUB_URL="https://api.github.com/repos/${GITHUB_ORGINIZATION}/${_REPOSITORY}/releases/latest"
    log_term 2 "Setting _GITHUB_URL to: \"${_GITHUB_URL}\""
    log_term 3 "File: '${BASH_SOURCE[0]}' Line: '${LINENO}'"

    # Get the previous release from github
    local PREVIOUS_RELEASE_VERSION; PREVIOUS_RELEASE_VERSION=$(collect_data "${_GITHUB_URL}" | jq --raw-output '.tag_name // empty')
    log_term 2 "Collecting PREVIOUS_RELEASE_VERSION"
    log_term 3 "File: '${BASH_SOURCE[0]}' Line: '${LINENO}'"

    # If we have an existing changelog, set it as the --base and edit it
    if [ -f "${CHANGELOG_FILE}" ]; then
        CHANGELOG_GENERATOR_COMMAND+=( '--base' "${CHANGELOG_FILE}" )
        # We are goign to remove some bits from the current CHANGELOG.md file
        #+ These bits are added by github_changelog_generator
        #+ and will be added back in when we run it below
        # Remove the header from the old CHANGELOG.md
        log_term 2 "Editing ${CHANGELOG_FILE} with '/^\# Change Log$/d'"
        log_term 3 "File: '${BASH_SOURCE[0]}' Line: '${LINENO}'"
        sed -i "/^\# Change Log$/d" "${CHANGELOG_FILE}"
        # Remove extra newline at the top
        log_term 2 "Editing ${CHANGELOG_FILE} with '1{/^$/d}'"
        log_term 3 "File: '${BASH_SOURCE[0]}' Line: '${LINENO}'"
        sed -i '1{/^$/d}' "${CHANGELOG_FILE}"
        # Remove The old generated by line at the bottom
         log_term 2 "Editing ${CHANGELOG_FILE} with '/^\\\\\\* \*This Change Log was automatically generated by \[github_changelog_generator\]/d'"
        log_term 3 "File: '${BASH_SOURCE[0]}' Line: '${LINENO}'"
       sed -i "/^\\\\\\* \*This Change Log was automatically generated by \[github_changelog_generator\]/d" "${CHANGELOG_FILE}"
        # Remove extra newlines from the end of the file
        while [ "$(tail -n 1 "${CHANGELOG_FILE}")" == "" ]; do
            log_term 2 "Editing ${CHANGELOG_FILE} with '$ {/^$/d}'"
            log_term 3 "File: '${BASH_SOURCE[0]}' Line: '${LINENO}'"
            sed -i '$ {/^$/d}' "${CHANGELOG_FILE}"
        done
        # Check to see if this is a re-release
        #+ If so, remove the current release block as we will regenerate it
        if [ "$(grep -c "\#\# \[${_RELEASE}\]" ${CHANGELOG_FILE})" != 0 ]; then
            # Look up the previous release
            #+ This overrides PREVIOUS_RELEASE_VERSION as we do not want
            #+ PREVIOUS_RELEASE_VERSION to equal RELEASE (it looks ugly)
            log_term 2 "Looks like a re-release! Looking up previous release number."
            log_term 3 "File: '${BASH_SOURCE[0]}' Line: '${LINENO}'"

            # Set up the GtHub API URL for the latest release
            local _GITHUB_URL="https://api.github.com/repos/${GITHUB_ORGINIZATION}/${_REPOSITORY}/releases"
            log_term 2 "Setting _GITHUB_URL to: \"${_GITHUB_URL}\""
            log_term 3 "File: '${BASH_SOURCE[0]}' Line: '${LINENO}'"

            # /bin/ash does not accept --long options to sort
            PREVIOUS_RELEASE_VERSION=$(collect_data "${_GITHUB_URL}" \
                | jq --raw-output .[].tag_name \
                | sort -t . -n -r \
                | head -2 \
                | tail -1)
            log_term 2 "Got ${PREVIOUS_RELEASE_VERSION} PREVIOUS_RELEASE_VERSION"
            log_term 3 "File: '${BASH_SOURCE[0]}' Line: '${LINENO}'"

            # Get the line number for the current release
            CURRENT_RELEASE_LINE="$(sed -n "/\#\# \[${_RELEASE}\]/=" "${CHANGELOG_FILE}" | grep -m 1 ^)"
            log_term 2 "Got ${CURRENT_RELEASE_LINE} CURRENT_RELEASE_LINE"
            log_term 3 "File: '${BASH_SOURCE[0]}' Line: '${LINENO}'"

            # Get the line number for the previous release
            PREVIOUS_RELEASE_LINE="$(sed -n "/\#\# \[${PREVIOUS_RELEASE_VERSION}\]/=" "${CHANGELOG_FILE}" | grep -m 1 ^)"
            log_term 2 "Got ${PREVIOUS_RELEASE_LINE} PREVIOUS_RELEASE_LINE"
            log_term 3 "File: '${BASH_SOURCE[0]}' Line: '${LINENO}'"

            # Delete all lines contained in the current release block
            log_term 2 "Editing ${CHANGELOG_FILE} with '${CURRENT_RELEASE_LINE},$(( PREVIOUS_RELEASE_LINE-1 ))d'"
            log_term 3 "File: '${BASH_SOURCE[0]}' Line: '${LINENO}'"
            sed -i -e "${CURRENT_RELEASE_LINE},$(( PREVIOUS_RELEASE_LINE-1 ))d" "${CHANGELOG_FILE}"
        fi
    fi
    # If we have a previous release set the --since-tag
    if [ ! -z "${PREVIOUS_RELEASE_VERSION}" ]; then
        CHANGELOG_GENERATOR_COMMAND+=( '--since-tag' "${PREVIOUS_RELEASE_VERSION}" )
    fi
    # Be explicate about the output filename
    CHANGELOG_GENERATOR_COMMAND+=( '--output' "${CHANGELOG_FILE}" )
    #+ In order to fetch fewer issues, and stop triggering GitHub's
    #+ malicious API detection we will only grab the last 100 issues.
    #+ This should be enough to cover the last release. We will then generate
    #+ only the changelog since the last release and prepend that to the
    #+ existing file.
    CHANGELOG_GENERATOR_COMMAND+=( '--max-issues' '100' )

    # Check the api limit before we start running the generator
    github_api_limit_check "${_API_REQUEST_COUNT}"

    log_term 2 "Running '${CHANGELOG_GENERATOR_COMMAND[*]} ${GITHUB_ORGINIZATION}/${_REPOSITORY}'"
    log_term 3 "File: '${BASH_SOURCE[0]}' Line: '${LINENO}'"
    "${CHANGELOG_GENERATOR_COMMAND[@]}" "${GITHUB_ORGINIZATION}"/"${_REPOSITORY}"
#    github_changelog_generator --token "${GITHUB_OATH_TOKEN}" --future-release "${_RELEASE}" "${GITHUB_ORGINIZATION}"/"${_REPOSITORY}"
    git add "${CHANGELOG_FILE}"
    git commit -m "Update ${CHANGELOG_FILE} for ${_RELEASE} release [skip ci]"
